...
if (config.github.requireTeam) {
   ...
   var getTeamId = github.getTeams(config.github.requireOrg)
   .then(function(teams) {
      ...
      if (!team) {
         ...
         debug(m);
         console.error(m);
         // TODO: This type of error probably should't take down the whole app.
         process.exit(1);
      }
      return team.id;
   });
   ...
   checkOrgMembershipRequirements = function(options) {
      return getTeamId.then(function(teamId) {
         return github.getTeamMembership({
            id: teamId,
   ...
   checkOrgMembershipRequirements = github.getOrgMembership;
}
